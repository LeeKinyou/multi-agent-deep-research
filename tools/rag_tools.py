...
使用crewai的@tool装饰器（与search_tool.py保持一致）
"""
...
def create_rag_tools(task_id: str) -> list:
    """创建RAG相关的crewai工具列表"""

    @tool("store_research_info")
    def store_information(text: str, source: str = "agent_output") -> str:
        """将采集到的研究信息存储到向量数据库中，供后续分析使用。

        Args:
            text: 要存储的文本内容
            source: 来源标识，默认为agent_output
        """
        try:
    ...
    return [store_information, search_information]
